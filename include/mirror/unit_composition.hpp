...
struct unit_composition<Unit, unpacked_metaobject_sequence<M...>>
  : Unit<wrapped_metaobject<M>>... {

    template <typename... A>
    constexpr unit_composition(A&... args)
      : Unit<wrapped_metaobject<M>>{args...}... {}
};

template <template <typename> class Unit, __metaobject_id M>
struct unit_composition<Unit, wrapped_metaobject<M>>
  : unit_composition<Unit, decltype(unpack(wrapped_metaobject<M>{}))> {
    using unit_composition<Unit, decltype(unpack(wrapped_metaobject<M>{}))>::
      unit_composition;
};
...
constexpr auto apply(
  ...
  F function) {
    return function(static_cast<Unit<wrapped_metaobject<M>>&>(cmp)...);
}
...
constexpr auto apply(
  ...
  F function) {
    return function(static_cast<const Unit<wrapped_metaobject<M>>&>(cmp)...);
}
...
constexpr void for_each(
  unit_composition<Unit, unpacked_metaobject_sequence<M...>>& cmp,
  F function) {
    (void)(..., function(static_cast<Unit<wrapped_metaobject<M>>&>(cmp)));
}
...
constexpr void for_each(
  const unit_composition<Unit, unpacked_metaobject_sequence<M...>>& cmp,
  F function) {
    (void)(..., function(static_cast<const Unit<wrapped_metaobject<M>>&>(cmp)));
}
...
constexpr auto make_array(
  ...
  -> std::array<
    std::common_type_t<decltype(
      transform(static_cast<const Unit<wrapped_metaobject<M>>&>(cmp)))...>,
    sizeof...(M)> {
    return {
      {transform(static_cast<const Unit<wrapped_metaobject<M>>&>(cmp))...}};
}
...
constexpr auto make_array_of(
  ...
  -> std::array<R, sizeof...(M)> {
    return {{R(static_cast<Unit<wrapped_metaobject<M>>&>(cmp))...}};
}
...
constexpr auto make_array_of(
  ...
  -> std::array<std::add_const_t<R>, sizeof...(M)> {
    return {{std::add_const_t<R>(
      static_cast<const Unit<wrapped_metaobject<M>>&>(cmp))...}};
}
