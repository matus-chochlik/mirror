/**
 *  .file test/mire/mirror/decl_type.cpp
 *  .brief Test case for compile-time decl_type meta-function
 *
...
#define BOOST_TEST_MODULE MIRE_MIRROR_type
...
BOOST_AUTO_TEST_SUITE(mire_mirror_type)
...
BOOST_AUTO_TEST_CASE(mire_mirror_type_test1)
{
	using namespace mire;
	using namespace mire::mirror;

	BOOST_CHECK((!equal<
		decl_type<mirrored_t<std::string>>,
		mirrored_t<std::string>
	>::value));

	BOOST_CHECK((nonequal<
		decl_type<mirrored_t<std::wstring>>,
		mirrored_t<std::wstring>
	>::value));

	BOOST_CHECK((std::is_same<
		original_type<decl_type<mirrored_t<std::string>>>::type,
		original_type<mirrored_t<std::string>>::type
	>::value));

	BOOST_CHECK((std::is_same<
		original_type<decl_type<mirrored_t<std::wstring>>>::type,
		original_type<mirrored_t<std::wstring>>::type
	>::value));
}

BOOST_AUTO_TEST_CASE(mire_mirror_type_test2)
{
	using namespace mire;
	using namespace mire::mirror;
...
BOOST_AUTO_TEST_CASE(mire_mirror_type_test3)
{
	using namespace mire;
	using namespace mire::mirror;
