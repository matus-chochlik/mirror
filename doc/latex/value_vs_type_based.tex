...
% ------------------------------------------------------------------------------
\begin{frame}
  \frametitle{Value- vs. type-based reflection}
  \makebox[\linewidth][c]{
...
\begin{frame}
  \frametitle{i5-2400}
  \framesubtitle{compile time increase per N metaobjects}
  \fitfig{ct-per-n-i5-2400}
\end{frame}
% ------------------------------------------------------------------------------
\begin{frame}
  \frametitle{i5-2400}
  \framesubtitle{compile time increase per 1 metaobject}
  \fitfig{ct-per-1-i5-2400}
\end{frame}
% ------------------------------------------------------------------------------
\begin{frame}
  \frametitle{i5-2400}
  \framesubtitle{How much faster is value-based vs. type-based}
...
% ------------------------------------------------------------------------------
\begin{frame}
  \frametitle{Representing the reflection \say{operator}}
  \Large
  \begin{itemize}
    \item Above we have just used integer literals to represent metaobject \say{ids}
    \item What if we used a template function\footnote{with NTTP} to simulate
      the reflection expression
    \item What would be the effects on the compile-times?
  \end{itemize}
\end{frame}
% ------------------------------------------------------------------------------
\begin{frame}[fragile]
  ...
  \begin{columns}
    ...
    \begin{lstlisting}[language=c++2x,basicstyle=\normalsize\ttfamily]
template <size_t I>
consteval auto (*@\listinghlt{reflect}@*)() {
    return I;
}
    \end{lstlisting}
    \begin{lstlisting}[language=c++2x,basicstyle=\normalsize\ttfamily]
foo((*@\listinghlt{reflect}@*)<(*@\listinghls{N}@*)Z>());
// and
foo<(*@\listinghlt{reflect}@*)<(*@\listinghls{N}@*)Z>()>();
    \end{lstlisting}
    \end{column}
  ...
  \begin{columns}
    ...
    \begin{lstlisting}[language=c++2x,basicstyle=\normalsize\ttfamily]
template <size_t I>
consteval (*@\listinghl{wrapper}@*)<I> (*@\listinghlt{reflect}@*)() {
  return {};
}
    \end{lstlisting}
    \begin{lstlisting}[language=c++2x,basicstyle=\normalsize\ttfamily]
...
\begin{frame}
  ...
  {\rowcolors{2}{mirrorpage}{darkgray}
    ...
    \hline
      type-based template      & 115.9s  &  48.8s  &  62.4s  \\
                               & 0.42\%  & 0.16\%  &  0.19\% \\
    \hline
      type-based consteval     & 111.3s  &  53.4s  &  62.7   \\
                               & 0.41\%  & 0.16\%  &  0.19\% \\
    \hline
      value-based template     &  36.3s  &  16.5s  &  19.0s  \\
                               & 0.13\%  & 0.05\%  &  0.06\% \\
    \hline
      value-based consteval    &  50.3s  &  27.4s  &  29.6s  \\
                               & 0.18\%  & 0.09\%  &  0.09\% \\
    \hline
    \end{tabular}
  }
