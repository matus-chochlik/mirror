...
% ------------------------------------------------------------------------------
\begin{frame}[fragile]
  \frametitle{Alternative API\footnote{pseudocode}}
  \begin{columns}
    \begin{column}{.50\textwidth}
      \begin{lstlisting}[language=c++2x,basicstyle=\scriptsize\ttfamily]
enum class trait {
...
void print_traits(metaobject auto mo) {
  ...
  cout << "traits of " << get_display_name(mo) << endl;
  for_each(mes, [&](metaobject auto me) {
      cout << "  " << get_name(me)
           << ": "
           << string(maxl - get_name(me).size(), ' ')
           << boolalpha
           << (*@\listinghlt{has_trait}@*)<(*@\listinghl{get_constant}@*)(me)>(mo) << endl;
  });
  cout << endl;
}
  \end{lstlisting}
