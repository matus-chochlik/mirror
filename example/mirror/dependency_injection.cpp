...
struct injector : TWirings... {
    ...
    constexpr auto create() const {
        return []<class... Ts>(std::tuple<Ts...>) {
            const auto wiring = []<class TName, class TParam>(
                ...
                constexpr auto has_named_wiring = []<class TWiring>(TWiring) {
                    return std::string_view{TWiring::name}.substr(
                             0,
                             std::size(
                               std::string_view{meta::get_name_v<TName>})) ==
                ...
                static_assert(
                  ((has_type_wiring(TWirings{}) or
                    has_named_wiring(TWirings{})) or
                   ...),
                  "Missing wiring!");
            ...
            return (
              [&]<class TName, class... TParams>(
                TName, std::tuple<TParams...>) {
                  return T{wiring(TName{}, TParams{})...};
              }(Ts{},
                meta::
                  unpack_sequence_t<std::tuple, meta::get_parameters_t<Ts>>{}),
              ...);
        }
...
template <class... TWirings>
injector(TWirings...) -> injector<TWirings...>;
...
int main() {
    ...
    struct dependency_injection {
        dependency_injection(const interface& pi1, const interface& pi2)
          : i1{pi1}
          , i2{pi2} {}
