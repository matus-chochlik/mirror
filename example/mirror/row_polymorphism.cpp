...
template <class... Ts>
struct rows : Ts... {
  template <class T> requires (has_member_name<T, Ts::name> and...)
  explicit(false) rows(const T& t) { (
    [&] {
      for_each(get_data_members(get_aliased(mirror(T))), [&](auto mo) {
        if (Ts::name == get_name(mo)) { Ts::value = get_value(mo, t); }
      });
    }(), ...);
  }

  template<class T>
  constexpr decltype(auto) operator[](T) { return get<T::name>(*this); }

 private:
  template<fixed_string Name, class T> constexpr decltype(auto) get(row<Name, T>& t) { return (t.value); }
};
